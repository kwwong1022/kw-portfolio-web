aboutBannerSketch = (s) => {
    ...
    s.setup = function() {
        s.createCanvas(document.querySelector('#'+aboutBannerSketchId).clientWidth, document.querySelector('#'+aboutBannerSketchId).clientHeight);
        canvasWidth = s.width;
    ...
    s.draw = function() {
        ...
        s.textSize(36);
        s.textFont(font);
        s.text('About', s.width/2, currHeight-35);
        s.textSize(16);
        s.textFont(fontB);
        s.text('Background, Skills, Education', s.width/2, currHeight);

        currHeight = s.lerp(currHeight, targetHeight, .1);
    }

    s.windowResized = function() {
        s.resizeCanvas(document.querySelector('#'+aboutBannerSketchId).clientWidth, document.querySelector('#'+aboutBannerSketchId).clientHeight);
        canvasWidth = s.width;
        canvasHeight = s.height;
        initCircles();
    }
...
let aboutBannerSketchId = 'about-banner-sketch';
var aboutBannerP5 = new p5(aboutBannerSketch, aboutBannerSketchId);